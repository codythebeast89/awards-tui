...
| III. UX Consistency | Esc-cancel convention; typed confirm phrase for destructive writes; CLI/TUI parity; theme-only colors | **PASS**, with one documented, justified gap | `Esc` from the Audit modal now has three constitution-documented cases (amended in v1.1.0 after this feature shipped, see Constitution III): closes without overwriting status from the list/report view; returns to the list from the `ChooseUsername` sub-dialog; and restores the stashed Audit modal with status "Fix cancelled" when backing out of a fix opened from a selected finding. Delete/Rename opened from a selected finding still require the existing typed `"delete"`/`"rename"` confirmation phrase — no new one-keypress destructive path is introduced. **CLI/TUI parity gap**: the CLI gains no new flag for this feature (research.md §5) — "select from a list" has no meaning in a one-shot CLI command, and the clerk already has equivalent one-shot flags (`--edit`/`--cell`, `--delete`, `--rename`) to act on anything the existing `--audit` report names. This is the explicit call-out the principle requires when parity isn't practical. New list rendering reuses the existing `ListState`/`List` widget pattern already used by `AddModal`, not a new UI pattern. |
